...
int Camera::create(int index,
  ...
  qDebug() << id;
  // 创建相机设备, 获取句柄
  CameraHandle handle = cameraCore->Gauss_CameraCore_CreateCamera(
      indexToCameraInfo[index].infoJson);
  indexToCameraInfo[index].handle = handle;
  qDebug() << "index: " << index << " " << handle;

  int res = 0;
  // 设置图像回调
  if ((res = cameraCore->Gauss_CameraCore_SetImageReady(handle, imageCall)) <
      0) {
    LOG;
...
int Camera::open(int index) {
  ...
  int res = 0;
  // 打开相机
  if ((res = cameraCore->Gauss_CameraCore_OpenCamera(
           indexToCameraInfo[index].handle)) < 0) {
    LOG;
...
int Camera::take(int index) {
  ...
  int res = 0;
  // 触发相机拍照
  if ((res = cameraCore->Gauss_CameraCore_TriggerOnce(
           indexToCameraInfo[index].handle,
           indexToCameraInfo[index].infoJson)) < 0) {
    LOG;
...
int Camera::close(int index) {
  qDebug() << "close()";
  // 关闭相机
  cameraCore->Gauss_CameraCore_CloseCamera(indexToCameraInfo[index].handle);
  // if (cameraCore->Gauss_CameraCore_CloseCamera(
  //         indexToCameraInfo[index].handle) < 0) {
  //   LOG;
