    ...
    , timerWorker(new Timer)
{
    ...
    connect(this, &MainWindow::SIGNAL_openCamera, camera, &Camera::open, Qt::QueuedConnection);
    connect(this, &MainWindow::SIGNAL_takePicture, camera, &Camera::take, Qt::QueuedConnection);
    cameraThread->start();

    // 串口线程
    serialThread = new QThread(this);
    serialPort->moveToThread(serialThread);
    connect(serialThread, &QThread::finished, serialPort, &QObject::deleteLater);
    connect(this, &MainWindow::SIGNAL_openSerial, serialPort, &SerialPort::open, Qt::QueuedConnection);
    connect(this, &MainWindow::SIGNAL_sendSerialData, serialPort, &SerialPort::send, Qt::QueuedConnection);
    serialThread->start();

    // Timer线程
    timerThread = new QThread(this);
    timerWorker->moveToThread(timerThread);
    connect(timerThread, &QThread::started, timerWorker, &Timer::startWork);
    connect(timerThread, &QThread::finished, timerWorker, &Timer::stopWork);
    connect(timerWorker, &Timer::workProgress, this, []() {
        qDebug() << "Work progress signal emitted from timer.";
    });
    timerThread->start();

    // 连接ui操作
    connect(ui->pushButton_cameraOpen, &QPushButton::clicked, this, [=]() {
        emit SIGNAL_openCamera(ui->comboBox_cameraList->currentIndex());
    });
    connect(ui->pushButton_cameraTake, &QPushButton::clicked, this, [=]() {
        emit SIGNAL_takePicture(ui->comboBox_cameraList->currentIndex());
    });
    connect(ui->pushButton_serialOpen, &QPushButton::clicked, this, [=]() {
        emit SIGNAL_openSerial(ui->comboBox_serialList->currentIndex());
    });
}
