...
//   unsigned int errnu =
//   commCore->Gauss_CommunicationCore_GetLastError(handle); qDebug() << "error:
//   " << QString(" %1 ").arg(errnu, 8, 16); LOG;
// }
...
//   std::string serialPortInfo =
//   doc.toJson(QJsonDocument::Compact).toStdString();

//   // 创建设备
//   int handle =
//   commCore->Gauss_CommunicationCore_CreateDevice(serialPortInfo);
...
//   int res = 0;
//   res =
//       commCore->Gauss_CommunicationCore_SetReadyReadCallBack(handle,
//       recvCall);
//   if (res != 0) {
//     debug(handle);
//   }
